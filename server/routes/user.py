...
async def login(user: UserLoginModel = Body(...)):
    ...
    if len(present_user) ==0 :
        raise HTTPException(status.HTTP_400_BAD_REQUEST, ErrorResponseModel("Error", 400, "User not found, try signing up"))
    if(compare_password(jsonable_encoder(user)["password"], present_user["password"])):
        ...
        raise HTTPException(status.HTTP_400_BAD_REQUEST, ErrorResponseModel("Error", 400, "Login credentials incorrect"))
...
async def get_users():
    ...
    raise HTTPException(status.HTTP_404_NOT_FOUND, ErrorResponseModel("Error", 404, "Database empty, there are no users yet"))
...
async def get_user_by_id(id):
    ...
    raise HTTPException(status.HTTP_404_NOT_FOUND, ErrorResponseModel("Error", 404, "User does not exist"))
...
async def update_user_data(id: str, req: UpdateUserModel = Body(...)):
    ...
    raise HTTPException(status.HTTP_400_BAD_REQUEST, ErrorResponseModel("Error", 400, "Cannot update user data"))
...
async def delete_user_data(id:str):
    ...
    raise HTTPException(status.HTTP_400_BAD_REQUEST, ErrorResponseModel("Error", 400, "Cannot delete user from database"))